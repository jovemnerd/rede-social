<?php
	...
	class JovemNerdBlogController extends Controller{
		...
		public function login(){
			...
			$v->link = HOST . "meu-perfil/redes-sociais/jovemnerd/callback/?uid={$uid}&token={$token}";
			$message = $v->render('mail/jovemnerdblog-link-account.phtml');
			...
			try{
				...
				$mail->Subject = 'SkyNerd: Vínculo de conta de email';
			...
			if($mail_status){
				...
				SocialNetwork::link_account($this->session->user->id, BLOG, $this->post->email_address, false);
				die('SUCCESS');
			} else {
				die('FAIL');
			}	
		...
		public function logout(){
			Phalanx::loadClasses('SocialNetwork');
			SocialNetwork::unlink_account($this->session->user->id, BLOG);
		...
		public function callback(){
			...
			if($expected_token == $this->get->token){
				Phalanx::loadClasses('SocialNetwork');
				SocialNetwork::activate_account($this->session->user->id, BLOG);
				Request::redirect(HOST.'perfil/configuracoes');
			} else {
				Request::redirect(HOST.'perfil/configuracoes');	
			}
